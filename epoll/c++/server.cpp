...
vector<Client_info> vec;

void onAccept(int sock);
void on_recv(int sock);
void extractInfo(char* info, int fd);
int isExit(Client_info client_info);
...
void extractInfo(char* info, int fd)
{
	...
	if (strncmp(info, "imei", strlen("imei")) ==0 ) {
		sscanf(info, "imei:%s info:%s", imei, recv_data);
		printf("imei:%s,info:%s \n", imei, recv_data);
	}
	client_info.client_fd = fd;
	strcpy(client_info.imei, imei);
	client_info.is_server = 0;
	if (isExit(client_info))
	{
		printf("have exist! \n");
	}
	else {
		vec.push_back(client_info);
	}
	printf("imei:%s,fd:%d \n", vec[0].imei, vec[0].client_fd);
...
{
	int res = 0;
	vector<Client_info>::iterator i;
	for (i = vec.begin(); i < vec.end(); i++)
	{
		printf("i :%d, imei:%s,fd:%d \n", i, i->imei, i->client_fd);
		if (strcmp(client_info.imei, i->imei) == 0 && (client_info.is_server == i->is_server)) {
			res = 1;
			//printf("is Same! \n");
			break;
		}
