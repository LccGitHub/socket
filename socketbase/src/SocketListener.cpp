...
int SocketListener::stopListener() {
    ...
    if (rc != 1) {
        SLOGE("Error writing to control pipe (%s)\n", strerror(errno));
        return -1;
    }

    void *ret;
    if (pthread_join(mThread, &ret)) {
        SLOGE("Error joining to listener thread (%s)\n", strerror(errno));
        return -1;
    }
    close(mCtrlPipe[0]);
    close(mCtrlPipe[1]);
...
void SocketListener::runListener() {
    ...
    while(1) {
        ...
        if (!mListen && mClients->size() == 0) {
            SLOGE("socket client need exit and retry connect .... \n");
            break;
        } 

        pthread_mutex_unlock(&mClientsLock);
        SLOGV("mListen=%d, max=%d, mSocketName=%s\n", mListen, max, mSocketName);
        if ((rc = select(max + 1, &read_fds, NULL, NULL, NULL)) < 0) {
            if (errno == EINTR)
                continue;
            SLOGE("select failed (%s) mListen=%d, max=%d\n", strerror(errno), mListen, max);
            sleep(1);
            continue;
        } else if (!rc)
            continue;
...
void SocketListener::sendBroadcast(void *msg, int len) {
    ...
    while (!safeList.empty()) {
        ...
        if (c->sendData(msg, len)) {
            SLOGW("Error sending broadcast (%s)\n", strerror(errno));
        }
        c->decRef();
    }
}
